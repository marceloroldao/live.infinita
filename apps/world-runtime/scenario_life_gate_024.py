...
def build_life_gate_024_world(
    *,
    episode_id: int = 0,
    wind_phase_id: str = "w_gust_a",
    barrier_phase_id: str = "b_clear",
) -> dict:
    """Reacquisition world: a passively forgotten proxy returns with the old relation."""
    world = deepcopy(
        build_life_gate_023_world(
    ...
    world["world_id"] = (
        f"life-gate-024-{wind_phase_id}-{barrier_phase_id}-episode-{episode_id}"
    )
    world["rules"]["higher_order_reacquisition"] = {
        "proxy_sensor_id": "s_regime_proxy_01",
        "proxy_band_id": "x0",
        "phase_1_proxy_context_phase": "b_clear",
        "phase_2_proxy_present": False,
        "phase_3_proxy_context_phase": "b_clear",
...
def initial_nov_needs_024(world: dict) -> NeedState:

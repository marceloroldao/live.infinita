...
def build_life_gate_022_world(
    *,
    episode_id: int = 0,
    wind_phase_id: str = "w_gust_a",
    barrier_phase_id: str = "b_clear",
) -> dict:
    """Regime-shift world for formerly plausible higher-order context evidence."""
    world = deepcopy(
    ...
    world["world_id"] = (
        f"life-gate-022-{wind_phase_id}-{barrier_phase_id}-episode-{episode_id}"
    )
    world["rules"]["higher_order_regime_shift"] = {
        "proxy_sensor_id": "s_regime_proxy_01",
        "proxy_band_id": "x0",
        "phase_1_context_phase": "b_clear",
        "phase_2_context_phase": "b_blocked",
        "observations_per_combination": 5,
    }
    return world


def initial_nov_needs_022(world: dict) -> NeedState:

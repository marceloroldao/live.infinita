...
class NpcStrategyExperience:
    """Learn empirical cost for targets and full composite strategies.

    Target evidence remains keyed by NPC + need + target + canonical context for
    backward compatibility. Composite evidence adds strategy_id to the key so
    direct, via_shelter and wait_then_direct can learn independently even when
    they pursue the same target.
    """
    ...
    def _strategy_key(
        ...
        return f"{cls._key(npc_id, need, target_id, context)}\u001f{strategy_id}"

    def _load(self) -> dict[str, Any]:
        if not self.path.exists():
            return {
                "schema": "npc_strategy_experience_v2",
                "entries": {},
                "outcomes": {},
                "strategy_entries": {},
                "strategy_outcomes": {},
        ...
        value.setdefault("schema", "npc_strategy_experience_v2")
        value.setdefault("entries", {})
        value.setdefault("outcomes", {})
        value.setdefault("strategy_entries", {})
        value.setdefault("strategy_outcomes", {})
        return value
    ...
    def observe_strategy(
        ...
        *,
        ...
        if not outcome_id:
            raise ValueError("outcome_id is required")
        outcomes = self._state.setdefault("strategy_outcomes", {})
        if outcome_id in outcomes:
            return deepcopy(outcomes[outcome_id])
        ...
        if not npc_id or not need or not target_entity_id or not strategy_id:
            raise ValueError("npc_id, need, target_entity_id and strategy_id are required")
        ...
        rep = max(0, int(replans))
        risk = min(1.0, max(0.0, float(observed_risk)))
        key = self._strategy_key(npc_id, need, target_entity_id, strategy_id, context)
        entries = self._state.setdefault("strategy_entries", {})
        current = entries.get(key) if isinstance(entries.get(key), dict) else {}
        count = int(current.get("count", 0))
        row = {
            "npc_id": npc_id,
            ...
            "context": self.canonical_context(context),
            "count": count + 1,
            "mean_satisfaction": self._mean(float(current.get("mean_satisfaction", 0.0)), count, satisfaction),
            "mean_elapsed_ticks": self._mean(float(current.get("mean_elapsed_ticks", 0.0)), count, float(elapsed)),
            "mean_preemptions": self._mean(float(current.get("mean_preemptions", 0.0)), count, float(pre)),
            "mean_replans": self._mean(float(current.get("mean_replans", 0.0)), count, float(rep)),
            "mean_observed_risk": self._mean(float(current.get("mean_observed_risk", 0.0)), count, risk),
            "last_outcome_id": outcome_id,
            "last_strategy_execution_id": str(strategy_execution_id or "").strip() or None,
            "last_terminal_plan_id": str(terminal_plan_id or "").strip() or None,
        }
        entries[key] = row
        result = {
            "outcome_id": outcome_id,
    ...
    def strategy_stats(
        ...
        key = self._strategy_key(str(npc_id), str(need).lower(), str(target_entity_id), str(strategy_id), context)
        row = self._state.get("strategy_entries", {}).get(key)
        if not isinstance(row, dict):
            return None
        result = deepcopy(row)
        result["empirical_ready"] = int(result.get("count", 0)) >= self.min_samples

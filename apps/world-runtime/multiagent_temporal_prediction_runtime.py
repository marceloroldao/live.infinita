...
from temporal_prediction_memoria_adapter import (
    resolve_agent_conditioned_sensor_future_set,
)


@dataclass(frozen=True, slots=True)
class MultiAgentTemporalPrediction:
    observer_id: str
    sensor_id: str
    current_band_id: str
    physical_futures: AgentConditionedSensorFutureSet
...
def _current_sensor_band(
    ...
    *,
    ...
    if not isinstance(reading, dict):
        raise ValueError("current sensor reading is required before multiagent prediction")
    band_id = str(reading.get("band_id") or "")
    if not band_id:
        raise ValueError("current sensor reading must contain band_id")
    return band_id


def predict_multiagent_environmental_sensor(
    ...
    *,
...
) -> MultiAgentTemporalPrediction:
    """Resolve world-agent causality first, temporal memory second."""
    current_band_id = _current_sensor_band(
        world,
        observer_id=observer_id,
        sensor_id=sensor_id,
    )
    physical_futures = preview_next_agent_conditioned_sensor_futures(
        world,
        observer_id=observer_id,
        sensor_id=sensor_id,
    )
    resolution = resolve_agent_conditioned_sensor_future_set(
        memory,
    ...
    return MultiAgentTemporalPrediction(
        observer_id=observer_id,
        sensor_id=sensor_id,
        current_band_id=current_band_id,

...
printf '\n[2/6] Instalando Server Audio...\n'
bash "$SOURCE_DIR/deploy/install-server-audio.sh"

printf '\n[3/6] Instalando Browser Audio e exportando Godot Showcase...\n'
bash "$SOURCE_DIR/deploy/install-browser-audio.sh"

printf '\n[4/6] Instalando renderer Godot nativo server-side...\n'
bash "$SOURCE_DIR/deploy/install-headless-renderer.sh"

printf '\n[5/6] Preparando Broadcaster (desabilitado)...\n'
bash "$SOURCE_DIR/deploy/prepare-broadcaster.sh"

printf '\n[6/6] Conferindo serviços e invariantes...\n'
...
if systemctl is-active --quiet live-infinita-broadcaster.service; then
  echo 'ERRO: Broadcaster não deveria estar ativo nesta fase.' >&2
  exit 1
fi
...
printf 'Broadcaster: preparado e desabilitado\n'
